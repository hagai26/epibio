...
#signals <- read.table(gzfile("big/GEO/GSE32079/GSE32079_non-normalized.txt.gz"),nrows=-1,header=TRUE,row.names=1,skip=0,sep='\t',dec = ".")
signals <- read.table("big/GEO/GSE32079/GSE32079_non-normalized.txt",nrows=-1,header=TRUE,row.names=1,skip=0,sep='\t',dec = ".")

# get info about samples in series

series.info <- read.table("global/GEO/joined/GSE32079.txt",sep='\t',row.names=1,header=TRUE)
