  ...
  for(type in type_levels) {
    for(study in study_levels) {
      ...
      if(sum(relevant_targets) > 0) {
        ...
        full_filename <- file.path(TCGA_folder, kind_output_filename)
...
idat_folder <- file.path(data_folder, "static\\L3_data\\idat")
...
splited_targets <- lapply(splited_targets, FUN=function(x) head(x, 30))  #XXX
...
ret <- lapply(grouped_chunked_targets, FUN=work_on_targets)
